...
- **Transition-fired zone banners flash ~1 frame — effectively invisible (dbe2621).** `World.ShowBanner` calls made INSIDE a `DoTransition` middle (e.g. `OnRunLost`'s `zone.retreat` "You fall back to the village.") render for ~1 video frame (~0.04s) then vanish. Root cause: those paths fire the banner while the HUD text layer is still hidden — `OnRunLost` re-enables only `_hudZone`/`_hudQuest` and never calls `World.SetTextVisible(true)` before `ShowBanner` (unlike `SpawnIntoChapter` which does call it at line ~783 before `zone.arrive`). So the banner's 2.2s `BannerFade` hold elapses while `HudRoot` is inactive; it only becomes visible as the coroutine clears it. In-world banners (chest loot via `TryInteract`, `OnBossDefeated`'s `zone.bossdown`/`chapdone`, crystal `end.notyet`) are NOT in a transition and hold ~2s normally — verified sustained. To capture a flashing banner, screenshot immediately on the transition landing or extract video frames (ffmpeg `-ss`); a post-fade still will miss it.
- **Zone banner text overflows the 432px portrait width** — long banners ("You fall back to the village.", "MOON SHARD hums in your hand") are clipped ~2 chars at each edge. `ZoneBanner` is `TextAlign.Center` under `HudRoot` and doesn't wrap/fit.
- **FLEE HOME / leave-to-title does NOT persist position** — `OnRunLost` retreats the hero to `VillageCenter` but never calls `SaveRun`, so CONTINUE reloads the pre-flee position (e.g. deep-forest boss approach, not the village). Same as `LeaveToTitle` — quit/leave without a manual SAVE loses the retreat.
