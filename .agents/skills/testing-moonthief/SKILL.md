...
## Round-18 (Linux-wipe→Linux-final) — touch cue + ERASE SAVE + end-card stats — verified
- **STALE-BINARY GOTCHA (important)**: the build dir the lead names may PREDATE the feature commits. `stat` the binary mtime vs `git log --format="%h %ci %s"` commit times — if the binary is older than the feature commit, it silently lacks the feature. Verify by grepping the build's `TheMoonThief_Data/resources.assets` for a feature string (`grep -ao "ERASE SAVE" ...resources.assets`). This round the named `Linux-wipe` build (16:53) predated the erase-save commit (17:01) — the real build was `Linux-final` (17:49). Also confirm the RUNNING process's cwd/exe (`readlink /proc/<pid>/exe`) so a stale window isn't mistaken for the target.
- **Touch cue** (`WorldView.DriveTouchCue`, runs every Update): ONE `TexArt.Glow()` sprite (scale 1.7x0.65, color 1/0.9/0.6, alpha `0.2+0.12*sin(6t)`≈0.08–0.32, sorting 1900) pools at the nearest in-range interactable's `pos.y-0.2`. Priority: `NearestNpc(1.4u)` > boss prop(1.6u) > `NearestChest(1.2u)` > `NearDoor(1.5u)`; `enabled=false` out of range. **EXTREMELY subtle** — a faint translucent warm ellipse; invisible in plain stills and drowned by NPC sprite-animation + chest cglow / boss bGlow / occupant lantern. Verify via **temporal pixel-diff** (20+ `import -window` frames → per-pixel max-min delta): a faint elliptical region pulses at a STATIC target's base (chest works best) vs flat/ambient noise out of range. NPC cue is code-identical but can't be isolated in stills under a moving villager. Door cue only shows during the ~1.4s post-exit `_doorCooldown` (auto-enter shares the 1.5u range).
- **ERASE SAVE** (`Frontend.RefreshSettingsRows`, title-side only): rows = textspeed/music/sound/shake/autobattle/**ERASE SAVE**/BACK (pause-side omits erase). Tap1 arms → label `set.erase.sure`="SURE? ONE MORE TAP"; BACK or re-enter (`RefreshSettingsRows(keepArm=false)`) resets to `set.erase`="ERASE SAVE". Armed tap2 → `SaveSystem.Erase()` + `ShowMain()` + `ShowToast("set.erased"="Save erased. The night is clear.")` + CONTINUE shows "NO SAVE" disabled + save file gone. Verify all: row present, arm label, reset, erase+toast+NO SAVE+`ls moonthief-save.json` → No such file.
- **End card** (`TriggerEnding` via interact within 2u of `CristalPos`=(11.5,7.5) village, `MoonShards>=4`): to reach it save-edit `shards:4`+`heroX/heroY` near crystal, continue, interact (Return). Stats line (`_endStats`, "LEVEL n  x FRIENDS  y GOLD") sits at fixed `y=-HalfH+6.6` under the epilogue — **a 3-line epilogue's last line overlaps it** (defect found). `KeyConfirm`=Return/Space/Z (NOT 'e').
- Interior exit mat `ExitPos`=(mid+1,y0-0.5)=(10,7.5), at the room's bottom; LeaveHouse on `dist<0.85`.

## Round-24-38 (Linux-v46) — icon/UI polish pass — verified + 2 minor defects
- **Icons verified everywhere**: title rows (moon/play/book/gear/info/share/star)+footer "PICK A ROW"; pause rows; journal hub rows; CHARACTER p1 (LEVEL star, XP scroll, MOON SHARDS moon, BEFRIENDED paw, BEASTS sword, CHESTS bag) + p2 (GOLD coin, ATK bolt, MAX HP shield, BLADE/CLOTH/CHARM); EQUIPMENT (blade/cloth/charm + ATK bolt + MAX HP shield); QUESTS (scroll + MAIN); SETTINGS (text/note/speaker/shake/auto/back — trash only on title-side erase); STORY cinema "TAP OR ENTER TO SKIP" + "n/9" counter; NEW GAME overwrite CONFIRM card "START OVER" check + "KEEP MY SAVE" red-X.
- **Battle labels/icons**: `Menu[0]` swaps per fighter style — AMBER→STRIKE+sword, SEA→SWEEP+bolt, MOSS→MEND+green-plus (Battle.cs:1175 `style==1?sweep:style==2?mend:strike`, `MenuIcon(13/29/0)`). BEFRIEND heart, LET GO hand. Verify by attacking through a party's turns.
- **DEFECT — MORSEL first-turn grey**: spec = MORSEL greys when `BestFood()==null` (0.35 alpha, Battle.cs:1179-82). With an EMPTY bag it renders WHITE on the battle's FIRST player turn but correctly greys on subsequent turns (verified pixel-brightness: white 227 on AMBER's opening turn, 0-bright/greyed on MOSS+SEA turns). The grey is applied in `BeginPlayerTurn` after `SetSelected` but doesn't hold on the opening turn. Minor.
- **DEFECT — turn chevron buried for party**: `_turnChev` (gold chevron over active fighter's head, Battle.cs:148-50,603,665-70) is positioned `Home.y + BodyHeight + 0.5`. But `rig.BodyHeight` is ONLY set for ENEMY rigs (line 414) — **party rigs keep BodyHeight=0**, so the chevron renders at `PartyFeet+0.5` = waist/feet level INSIDE the sprite, not over the head. No gold "v" visibly bobs over AMBER/MOSS during their turns. Minor.
- **BESTIARY quirk**: met species row (MOSS SLIME) shows name+"HP n ATK a-b" + a green-slime portrait (`TexArt.MapMonster` via `_pageIconSprites`; `Seen.ContainsKey`), unmet rows show "?????" with no portrait — correct. But the portrait sprite hangs at the met row's lower-left edge, slightly overlapping the row below — minor mis-anchor.
- **Menu coordinate gotcha**: xdotool `--window` coords are REAL window px (576x1024), not the 360x640 preview scale — multiply preview coords by 1.6. BACK/bottom rows are easy to miss. Use keyboard Down+Return for sub-pages instead of clicks.
- **Wild encounter refresh**: `TouchedMonster()` (walk into a wandering monster) or rare ambient `heroPos.y>20 && rand<0.0006` per frame. Field monsters spawn y15-57 (fields) / y61-76 (forest, ch2+), >10u from village (30,6). Reliable: teleport hero to ~y45 and walk a sweep — a slime touches within ~10s.
- Player.log: 0 exceptions for the whole pass.
