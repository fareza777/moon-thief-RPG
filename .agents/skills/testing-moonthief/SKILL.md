...
- **No per-crossing zone banner exists** — `NoteZone` only tracks visited zones; no ShowBanner fires when crossing y26/y58. The only "zone banner" is the chapter-spawn `zone.arrive.N` (fires on NEW GAME/CONTINUE/chapter transition, not on walking a boundary). Entering a house fires the `InteriorNameKey` banner instead.
...
- **Arrive banner can get STUCK on screen — OPEN edge-case defect (found 3fccf99).** If you enter a house while a `ZoneBanner` is still fading (~2.8s window), `EnterHouse`'s `World.gameObject.SetActive(false)` kills the `BannerFade` coroutine mid-flight → the banner freezes. On `LeaveHouse` the WorldView re-shows the frozen label → stays up indefinitely until the next `ShowBanner` replaces it (self-clears on next banner). Repro: teleport hero ONTO a door cell (e.g. 16.5,9.2), CONTINUE → auto-enters on load while arrive banner is fresh → exit mat → "NIGHT ONE / quiet hollow" stuck >15s. Affects ANY WorldView-deactivating transition during a banner fade (house, battle `World.SetActive(false)`, title). Fix idea: clear/re-fire the banner on re-show, or guard BannerFade against deactivation.
